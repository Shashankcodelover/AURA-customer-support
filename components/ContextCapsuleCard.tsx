    ...
    <>
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 rounded-lg bg-white/5 hover:bg-cyan-500/20 border border-white/10 hover:border-cyan-500/40 text-gray-400 hover:text-cyan-300 transition text-xs flex items-center gap-1"
            >
              <FileText size={13} />
            </button>
        ...
        <div className="flex flex-wrap items-center justify-between text-xs text-gray-400 border-y border-white/5 py-2.5 gap-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-500">Account Exposure:</span>
            <span className="px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/30 text-purple-300 font-mono text-[11px]">
              {ltvDisplay}
            </span>
          </div>
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {capsule.attemptedActions.slice(0, 4).map((a, i) => (
              <div
                key={i}
                className="flex items-start gap-2 text-xs text-gray-300 bg-black/20 rounded-lg p-2 border border-white/5"
              >
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="glass-card p-6 max-w-2xl w-full bg-[#0a0f1d] border border-cyan-500/40 space-y-5 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <div className="flex items-center gap-2">
                <FileText size={18} className="text-cyan-400" />
                <div>
                  <h3 className="font-bold text-white text-base">Executive Incident Brief</h3>
                  <p className="text-xs text-gray-400 font-mono">INC-{capsule.id.slice(0, 10).toUpperCase()}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => window.print()}
                  className="px-3 py-1.5 rounded-xl bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 text-xs font-semibold flex items-center gap-1.5 hover:bg-cyan-500/30 transition"
                >
            ...
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-black/40 p-3 rounded-xl border border-white/5">
                <div>
                  <span className="text-gray-500 block">Customer</span>
                  <strong className="text-white">{capsule.customerName}</strong>
                </div>
                <div>
                  <span className="text-gray-500 block">Tier / Value</span>
                  <strong className="text-purple-300">{capsule.customerTier} ({ltvDisplay})</strong>
                </div>
                <div>
                  <span className="text-gray-500 block">SLA Priority</span>
                  <strong className="text-rose-400">{capsule.urgency} Urgency</strong>
                </div>
                <div>
                  <span className="text-gray-500 block">Root-Cause Confidence</span>
                  <strong className="text-emerald-400">{Math.round(capsule.confidence * 100)}%</strong>
                </div>
              </div>

              <div>
                <h4 className="font-bold text-gray-300 uppercase tracking-wider text-[11px] mb-1">
                  Customer Incident Message
                </h4>
                <p className="text-gray-200 bg-white/5 p-2.5 rounded-lg border border-white/5 italic">
              ...
              <div>
                ...
                <p className="text-gray-200 bg-white/5 p-2.5 rounded-lg border border-white/5">
                  {capsule.rootCause}
                </p>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-1 list-disc list-inside text-gray-300 bg-white/5 p-2.5 rounded-lg border border-white/5">
              ...
              <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-3">
                <h4 className="font-bold text-cyan-300 uppercase tracking-wider text-[11px] mb-0.5">
                  Remediation Protocol
                </h4>
                <p className="text-cyan-100">{capsule.recommendedAction}</p>
              </div>
            </div>
